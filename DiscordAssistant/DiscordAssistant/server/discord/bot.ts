...
const client = new Client({
  intents: [
    ...
    GatewayIntentBits.GuildVoiceStates,  // Potřebné pro hlasové funkce
    GatewayIntentBits.GuildMembers,      // Potřebné pro získání informací o členech
    GatewayIntentBits.DirectMessages     // Pro možnost odpovídat do DM
  ],
});
...
export async function initializeBot() {
  ...
  try {
    ...
    // Update bot status
    if (config) {
      const statusMsg = typeof config.statusMessage === 'string' ? config.statusMessage : "Serving commands!";
      updateBotStatus(config.status as PresenceStatusData, statusMsg);
    }
...
export function isBotReady(): boolean {
  // Pokud vidíme v logu, že bot je připojen, ale client.isReady() nevrací true,
  // můžeme použít jen isInitialized jako indikátor stavu
